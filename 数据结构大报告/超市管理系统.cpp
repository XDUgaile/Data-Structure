...
void Admin_save(alklist &head){
	apointer p;
	ofstream ofile("C:\Shop_Manager_System-master\管理员信息.txt",ios::out);
...
alklist Admin_get(alklist &head){
	...
	head->next=NULL;
	p=head;
	ifstream ifile("C:\Shop_Manager_System-master\管理员信息.txt",ios::in);
	if(!ifile){
...
void custom_save(clklist &head){
	cpointer p;
	ofstream ofile("C:\Shop_Manager_System-master\普通用户信息.txt",ios::out);
...
clklist custom_get(clklist &buyer){
	...
	buyer->next=NULL;
	p=buyer;
	ifstream ifile("C:\Shop_Manager_System-master\普通用户信息.txt",ios::in);
	if(!ifile){
...
void Good_get(sqlist &Goods){
	ifstream ifile("C:\Shop_Manager_System-master\商品信息.txt",ios::in);
...
void Good_save(sqlist &Goods){
	ofstream ofile("C:\Shop_Manager_System-master\商品信息.txt",ios::out);
...
void Good_add(sqlist &Goods){
	...
	fstream ifile("C:\Shop_Manager_System-master\商品信息.txt",ios::in);
	for(;flat!=0;) {
		cout<<"请输入商品编号:";
		cin>>ID;
